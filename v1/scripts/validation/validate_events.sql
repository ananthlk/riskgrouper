...
SELECT
    COUNT(*) AS total_rows,
    COUNT(DISTINCT member_id || '|' || event_date || '|' || event_type || '|' || COALESCE(claim_id, '') || '|' || COALESCE(TO_VARCHAR(claim_line_number), '')) AS distinct_key_rows,
    (COUNT(*) - COUNT(DISTINCT member_id || '|' || event_date || '|' || event_type || '|' || COALESCE(claim_id, '') || '|' || COALESCE(TO_VARCHAR(claim_line_number), ''))) AS duplicate_row_count
FROM IDENTIFIER($TARGET_TABLE);
